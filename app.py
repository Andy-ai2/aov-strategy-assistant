...
def quick_edit_panel(name: str):
    ...
    with cols[1]:
        ...
        countered_by = st.text_input("被克制（逗號或空白分隔）",
                                     " ".join(h["countered_by"]),
...
with tab1:
    ...
    with right:
        ...
        if picked and picked in data and picked != "（請選擇）":
            ...
            countered_by = st.text_input("被克制（以逗號或空白分隔）",
                                         " ".join(h["countered_by"]),
                ...
                if st.button("🗑️ 刪除該英雄", key=f"btn_delete_{picked}"):
                    ...
                    for cname in list(comps.keys()):
                        ...
                        if comps[cname].get("core","") == picked:
                            comps[cname]["core"] = ""
                            changed = True
                        counters = comps[cname].get("counters", [])
                        if picked in counters:
                            comps[cname]["counters"] = [x for x in counters if x != picked]
                            changed = True
                    if changed:
